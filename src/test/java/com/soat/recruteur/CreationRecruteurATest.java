...
import com.soat.planification_entretien.archi_hexa.application.controlleur.RecruteurController;
...
public class CreationRecruteurATest extends ATest {
    ...
    public void leRecruteurEstCorrectementEnregistréAvecSesInformationsEtAnsDExpériences(String language, String email, int experienceEnAnnees) {
        ...
        final DBRecruteur recruteur = recruteurRepository.findById(recruteurId).get();
        assertThat(recruteur).usingRecursiveComparison()
                .ignoringFields("id")
                .isEqualTo(new DBRecruteur(language, email, experienceEnAnnees));
    }
    ...
    public void leRecruteurNEstPasEnregistré() {
        final Optional<DBRecruteur> recruteur = recruteurRepository.findById(recruteurId);
